...
from verl.trainer.main_ppo import create_rl_dataset, create_rl_sampler, trainer_dict_to_dataclass
...
class TaskRunner:
    def run(self, config):
        ...
        from .aysnc_ray_trainer import ResourcePoolManager, Role
        ...
        if n_gpus_rollout is None:
            raise ValueError("rollout.n_gpus must be specified in the config")
        if not (0 < n_gpus_rollout < total_gpus):
            raise ValueError(f"rollout.n_gpus ({n_gpus_rollout}) must be > 0 and < total_gpus ({total_gpus})")

        n_gpus_actor = total_gpus - n_gpus_rollout
        ...
        if n_gpus_rollout % config.trainer.n_gpus_per_node == 0:
            ...
            raise ValueError(
                f"rollout.n_gpus ({n_gpus_rollout}) must be divisible by either "
                f"n_gpus_per_node ({config.trainer.n_gpus_per_node}) or "
