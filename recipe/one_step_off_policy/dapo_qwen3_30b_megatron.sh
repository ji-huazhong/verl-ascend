...
MODEL_PATH=${MODEL_PATH:-"pretrained_models/Qwen3-30B-A3B"}
MCORE_MODEL_PATH="mcore/Qwen3-30B-A3B"
CKPTS_DIR=${CKPTS_DIR:-"/sfs_turbo/verl_716/ckpts/${project_name}/${exp_name}"}
TRAIN_FILE=${TRAIN_FILE:-"dapo-math-17k.parquet"}
TEST_FILE=${TEST_FILE:-"dapo-math-17k.parquet"}
# Algorithm
temperature=1.0
top_p=1.0
...
cp $0 $path_log_dir/
nohup ray job logs -f $ray_name 2>&1 | tee $path_log_dir/baymax.log &
